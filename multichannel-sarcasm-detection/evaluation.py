...
def evaluateClassification(list_t, list_prob_pred):
    ...
    f1_binary = f1_score(list_t, list_p, average='binary', pos_label=1, zero_division=0)
    prob_arr = np.array(list_prob_pred)  # [N, n_class]
    auc_list = []
    for i in range(n_class):
        fpr, tpr, thresholds = roc_curve(list_t, prob_arr[:, i], pos_label=i)
        auc_list.append(auc(fpr, tpr))

    return {'c_m': c_m, 'acc': acc, 'f1_macro': f1_macro, 'f1_binary': f1_binary,
            'pre_macro': pre_macro, 'rec_macro': rec_macro, 'f1_micro': f1_micro,
