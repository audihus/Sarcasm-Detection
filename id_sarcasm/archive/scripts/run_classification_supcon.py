...
class DataTrainingArguments:
    ...
    do_supcon_loss: bool = field(default=False, metadata={"help": "Use Supervised Contrastive Loss (Gunel et al., ICLR 2021)."})
    supcon_lambda: float = field(default=0.9, metadata={"help": "Mixing weight: loss = (1-lambda)*CE + lambda*SupCon (default 0.9)."})
    supcon_temperature: float = field(default=0.3, metadata={"help": "SupCon temperature tau (default 0.3, Gunel et al.)."})
...
def main():
    ...
    class SupConLoss(nn.Module):
        """Supervised Contrastive Loss (Khosla 2020; Gunel 2021).
        Menarik sampel sekelas saling mendekat, mendorong beda-kelas menjauh,
        di ruang representasi CLS ter-normalisasi. Tanpa projection head
        (sesuai formulasi Gunel untuk fine-tuning BERT).
        """
        ...
        def forward(self, features: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
            feats = nn.functional.normalize(features, dim=-1)         # [B, H]
            sim = torch.matmul(feats, feats.T) / self.temperature     # [B, B]
            sim = sim - sim.max(dim=1, keepdim=True).values.detach()  # stabilitas numerik
            B = feats.size(0)
            self_mask = torch.eye(B, device=feats.device, dtype=torch.bool)
            exp_sim = torch.exp(sim).masked_fill(self_mask, 0.0)
            lbl = labels.view(-1, 1)
            pos_mask = (lbl == lbl.T) & (~self_mask)
            log_prob = sim - torch.log(exp_sim.sum(dim=1, keepdim=True) + 1e-12)
            pos_count = pos_mask.sum(dim=1)
            valid = pos_count > 0
            if valid.sum() == 0:
                return torch.zeros((), device=feats.device)
            loss_i = -(log_prob * pos_mask).sum(dim=1) / pos_count.clamp(min=1)
            return loss_i[valid].mean()

    class WeightedTrainer(Trainer):
        def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
            labels = inputs.pop("labels")
            outputs = model(**inputs, output_hidden_states=data_args.do_supcon_loss)
            logits = outputs.get("logits")
            flat_logits = logits.view(-1, self.model.config.num_labels)
            flat_labels = labels.view(-1)
            if data_args.do_supcon_loss:
                ce = nn.CrossEntropyLoss()(flat_logits, flat_labels)
                cls = outputs.hidden_states[-1][:, 0, :]
                scl = SupConLoss(temperature=data_args.supcon_temperature)(cls, flat_labels)
                loss = (1.0 - data_args.supcon_lambda) * ce + data_args.supcon_lambda * scl
            elif data_args.do_focal_loss:
