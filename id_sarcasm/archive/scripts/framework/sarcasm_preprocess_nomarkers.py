...
def surface_markers(text):
    ...
    for tok in text.split():
        ...
        if len(core) > 1 and core.isupper():             # ALL CAPS
            tok = tok.lower()
            flags.append("[CAPS]")
        out.append(tok)
...
def to_encoder_text(raw):
    # VARIAN NO-MARKERS (ablation): SEMUA marker dimatikan.
    # surface_markers (CAPS/ELONG/REPPUNC) dan EMOTICON mapping dilewati.
    # Langkah lain dipertahankan persis: repair, demojize, collapse placeholder, tidy.
    # Tujuan: isolasi kontribusi marker. Yang berubah HANYA marker, bukan encoding/placeholder.
    t = repair_encoding(raw)
    t = emoji.demojize(t, delimiters=(" :", ": "))       # emoji -> alias (tetap, seperti hero)
