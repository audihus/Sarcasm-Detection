...
class DataTrainingArguments:
    ...
    use_context_sep: bool = field(
        default=False,
        metadata={"help": "Split [CTX]/[TGT] markers and encode as two sequences with [SEP]."}
    )
...
def main():
    ...
    def preprocess_function(examples):
        ...
        # Tokenize the texts
        if data_args.use_context_sep:
            encoded_list = []
            for text in examples["sentence"]:
                if text.startswith("[CTX]"):
                    parts = text.split("[TGT]", 1)
                    parent = parts[0].replace("[CTX]", "").strip()
                    comment = parts[1].strip() if len(parts) > 1 else text
                    enc = tokenizer(parent, comment, padding=False, max_length=max_seq_length, truncation="only_first")
                else:
                    enc = tokenizer(text, padding=False, max_length=max_seq_length, truncation=True)
                encoded_list.append(enc)
            result = tokenizer.pad(encoded_list, padding=padding, max_length=max_seq_length)
        else:
