...
""" Fine-tuning IndoBERT with span-pooling + InferSent-style interaction head.

Input format (built from `text` and `parent_text` columns):
  With parent : [CLS] [HAS_CTX] [CTX] <parent tokens> [TGT] <comment tokens> [SEP]
  Without     : [CLS] [NO_CTX] [TGT] <comment tokens> [SEP]

Classification head:
  mean-pool(parent span) + mean-pool(comment span) -> InferSent concat (4 x 768)
  -> Linear(3072, 768) -> Tanh -> Dropout(0.1) -> Linear(768, num_labels)
"""
...
class IndoBertInteractionForSequenceClassification(BertPreTrainedModel):
    """IndoBERT encoder + span-pooling + InferSent-style interaction head."""
    ...
    def __init__(self, config):
        ...
        self.ctx_token_id = getattr(config, "ctx_token_id", None)
        self.tgt_token_id = getattr(config, "tgt_token_id", None)
        self.sep_token_id = getattr(config, "sep_token_id", 102)

        # Applies BERT-style init (normal std=initializer_range, zero bias) to classifier
        self.post_init()
    ...
    def forward(
        ...
        h_parents, h_comments = [], []
        for i in range(B):
            ids = input_ids[i]
            h = hidden[i]

            ctx_pos = (ids == self.ctx_token_id).nonzero(as_tuple=True)[0]
            tgt_pos = (ids == self.tgt_token_id).nonzero(as_tuple=True)[0]
            sep_pos = (ids == self.sep_token_id).nonzero(as_tuple=True)[0]

            idx_ctx = ctx_pos[0].item() if len(ctx_pos) > 0 else None
            idx_tgt = tgt_pos[0].item() if len(tgt_pos) > 0 else None
            idx_sep = sep_pos[0].item() if len(sep_pos) > 0 else None

            # Comment span: (idx_tgt+1) to (idx_sep-1) inclusive
            if idx_tgt is not None and idx_sep is not None:
                cs, ce = idx_tgt + 1, idx_sep
                h_c = h[cs:ce].mean(0) if cs < ce else torch.zeros(H, device=device, dtype=h.dtype)
            else:
                h_c = torch.zeros(H, device=device, dtype=h.dtype)

            # Parent span: (idx_ctx+1) to (idx_tgt-1) inclusive; zero if no [CTX]
            if idx_ctx is not None and idx_tgt is not None:
                ps, pe = idx_ctx + 1, idx_tgt
                h_p = h[ps:pe].mean(0) if ps < pe else torch.zeros(H, device=device, dtype=h.dtype)
            else:
                h_p = torch.zeros(H, device=device, dtype=h.dtype)
...
def main():
    ...
    send_example_telemetry("run_classification_interaction", model_args, data_args)
    ...
    # Load config and tokenizer, extend tokenizer FIRST so token IDs are known
    ...
    # Extend tokenizer and stamp token IDs onto config BEFORE loading model,
    # so IndoBertInteractionForSequenceClassification.__init__ reads them from config.
    special_tokens = ["[HAS_CTX]", "[NO_CTX]", "[CTX]", "[TGT]"]
    tokenizer.add_special_tokens({"additional_special_tokens": special_tokens})
    config.ctx_token_id = tokenizer.convert_tokens_to_ids("[CTX]")
    config.tgt_token_id = tokenizer.convert_tokens_to_ids("[TGT]")
    config.sep_token_id = tokenizer.sep_token_id

    model = IndoBertInteractionForSequenceClassification.from_pretrained(
    ...
    def preprocess_function(examples):
        ...
        if data_args.text_column_names is not None:
            comment_col = data_args.text_column_names.split(",")[0]
            comments = examples[comment_col]
        else:
            comments = examples["sentence"]

        # `parent_text` column may be absent for augmented/non-context splits
        parents = examples.get("parent_text", [None] * len(comments))

        sentences = []
        for comment, parent in zip(comments, parents):
            parent_str = (parent or "").strip()
            if parent_str:
                sentences.append(f"[HAS_CTX] [CTX] {parent_str} [TGT] {comment}")
            else:
                sentences.append(f"[NO_CTX] [TGT] {comment}")

        result = tokenizer(sentences, padding=padding, max_length=max_seq_length, truncation=True)
